...
public class MainActivity extends AppCompatActivity {
    ...
    private void classifyImage(Bitmap image) {
        try {
            ...
            result.setText(lines[maxPos]);

            String s = "";
            String r = "";
            r += String.format("%.1f%%", confidences[maxPos] * 100);

            for (int i = 0; i < lines.length; i++) {
                s += String.format("%s: %.1f%%\n", lines[i], confidences[i] * 100);
            }

            confidence.setText(s);

            Toast.makeText(this, lines[maxPos] + " " +r, Toast.LENGTH_SHORT).show();
